...
async def add_task(message: Message):
    try:
      ...
      filepath2, og = await encode(filepath)
      if filepath2:
        await msg.edit("**⬆️ Starting To Upload**")
        thumb = await get_thumbnail(filepath2)
        width, height = await get_width_height(filepath)
        duration2 = await get_duration(filepath2)
        await msg.edit("**⬆️ Uploading Video ⬆️**")
        await app.send_video(video=filepath2, chat_id=message.chat.id, supports_streaming=True, file_name=og, thumb=thumb, duration=duration2, width=width, height=height, caption=og)
        os.remove(filepath)
        os.remove(thumb)
