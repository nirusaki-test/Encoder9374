...
def add_task(message: Message):
    try:
      ...
      if new_file:
        ...
        app.send_video(new_file, file_name=og, thumb=thumb, duration=duration, width=width, height=height, caption="SAMPLE")
        app.send_video(filepath, file_name=og, thumb=thumb, duration=duration2, width=width, height=height, caption=og)
        os.remove(new_file)
        os.remove(filepath)
        os.remove(thumb)
